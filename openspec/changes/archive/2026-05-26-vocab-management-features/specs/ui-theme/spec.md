...
The system SHALL support two visual themes: a Bright Theme (warm cream, peach, and soft teal colors) and a Dark Theme (slate, charcoal, and neon teal colors). The system SHALL apply the selected theme globally across all screens and components.

#### Scenario: User toggles theme
- **WHEN** the user clicks the theme toggle button in the header
- **THEN** the system SHALL smoothly transition the entire visual interface to the opposite theme (Bright to Dark, or Dark to Bright) by swapping CSS variables on the root document
